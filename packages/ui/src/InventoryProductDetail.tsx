...
export function InventoryProductDetail({ product, onBack, actions, onRestock, getMedicationHref }: InventoryProductDetailProps) {
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-semibold text-[var(--ink)] mb-1">{product.productName}</h1>
      <p className="text-xs text-[var(--faint)] font-mono mb-6">{product.id}</p>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="p-5">
          <h2 className="text-base font-semibold text-[var(--text)] mb-2">Stock</h2>
          <InfoRow label="Current level">
            <span className={`font-semibold tabular-nums ${product.isBelowThreshold ? 'text-[var(--danger)]' : 'text-[var(--ink)]'}`}>
              {product.stockLevel}
          ...
          <InfoRow label="Status">
            {product.isBelowThreshold
              ? <span className="text-[var(--danger)] font-medium">Below threshold</span>
              : <span className="text-[var(--ok)] font-medium">Adequate</span>
            }
          </InfoRow>

          {onRestock && (
            <div className="mt-4 pt-4 border-t border-[var(--border)]">
              <h3 className="text-sm font-medium text-[var(--text)] mb-3">Restock</h3>
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  <label className="block text-xs text-[var(--muted)] mb-1">Units to add</label>
                  <input
                    type="number"
                    min={1}
                    value={qty}
                  ...
                  />
                  <p className="text-xs text-[var(--faint)] mt-1">New total: {product.stockLevel + qty}</p>
                </div>
                <div className="pt-5">
              ...
              {success && <p className="text-xs text-[var(--ok)] mt-2">Stock updated successfully.</p>}
            </div>
          )}
        </Card>
      </div>
