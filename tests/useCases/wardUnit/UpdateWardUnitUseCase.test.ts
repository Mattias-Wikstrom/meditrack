...
import { UpdateWardUnitUseCase } from '../../../src/domain/wardUnit/useCases/UpdateWardUnitUseCase';
...
describe('UpdateWardUnitUseCase', () => {
  ...
  beforeEach(() => {
    ...
    wardUnitRepo.save(new WardUnit('ward-1' as WardUnitId, 'Ward One'));
    ...
    useCase = new UpdateWardUnitUseCase(wardUnitRepo, actorRepo, transactor, new SimpleEventBus());
  });

  it('updates the name and writes an audit entry', async () => {
    const result = await useCase.execute({
      requestingActorId: 'admin-1',
      id: 'ward-1',
      name: 'Ward One Renamed',
    });

    expect(result.successful).toBe(true);
    if (!result.successful) return;
    expect(result.value.name).toBe('Ward One Renamed');
    expect((await wardUnitRepo.findById('ward-1' as WardUnitId))?.name).toBe('Ward One Renamed');
    expect(auditRepo.getEntries()[0]?.action).toBe('WardUnitUpdated');
    expect(auditRepo.getEntries()[0]?.actorId).toBe('admin-1');
    expect(auditRepo.getEntries()[0]?.entityId).toBe('ward-1');
  });

  it('fails when the requesting actor is not found', async () => {
    const result = await useCase.execute({ requestingActorId: 'unknown', id: 'ward-1', name: 'X' });
  ...
  it('fails when the requesting actor is not an admin', async () => {
    const result = await useCase.execute({ requestingActorId: 'nurse-1', id: 'ward-1', name: 'X' });
  ...
  it('fails when the ward unit is not found', async () => {
    const result = await useCase.execute({ requestingActorId: 'admin-1', id: 'unknown', name: 'X' });
