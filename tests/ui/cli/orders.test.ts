...
describe('listOrders', () => {
  it('prints a message when there are no orders', () => {
    const output = new RecordingOutput();

    listOrders(new InMemoryOrderRepository(), output);

    expect(output.messages).toEqual(['No orders.']);
  });

  it('prints one line per order', () => {
    const repo = new InMemoryOrderRepository();
    const useCase = new CreateOrderUseCase(repo, new SimpleEventBus());
    useCase.execute({ actorId: 'cli', wardUnitId: 'ward-1' as WardUnitId, lines: [{ medicationId: 'med-1' as MedicationId, quantity: 5 }] });
    useCase.execute({ actorId: 'cli', wardUnitId: 'ward-1' as WardUnitId, lines: [{ medicationId: 'med-2' as MedicationId, quantity: 3 }] });
    const output = new RecordingOutput();

    listOrders(repo, output);
...
describe('createOrder', () => {
  it('prints the created order ID on success', () => {
    const repo = new InMemoryOrderRepository();
    const useCase = new CreateOrderUseCase(repo, new SimpleEventBus());
    const output = new RecordingOutput();

    createOrder(useCase, output, 'ward-1', 'med-1', 5);

    expect(output.messages).toHaveLength(1);
    expect(output.messages[0]).toContain('created');
  });

  it('exits with code 1 and prints an error on validation failure', () => {
    const useCase = new CreateOrderUseCase(new InMemoryOrderRepository(), new SimpleEventBus());
    const output = new RecordingOutput();

    expect(() => createOrder(useCase, output, 'ward-1', 'med-1', 0)).toThrow(ExitError);
    expect(output.errors[0]).toContain('OrderLineQuantitiesPositive');
  });
});

describe('advanceOrder', () => {
  it('prints the new status on success', () => {
    const repo = new InMemoryOrderRepository();
    const eventBus = new SimpleEventBus();
    const created = new CreateOrderUseCase(repo, eventBus).execute({ actorId: 'cli', wardUnitId: 'ward-1' as WardUnitId, lines: [{ medicationId: 'med-1' as MedicationId, quantity: 5 }] });
    if (!created.successful) throw new Error('setup failed');
    const useCase = new AdvanceOrderStatusUseCase(repo, eventBus);
    const output = new RecordingOutput();

    advanceOrder(useCase, output, created.value.id);

    expect(output.messages[0]).toContain('Sent');
  });

  it('exits with code 1 for an unknown order', () => {
    const useCase = new AdvanceOrderStatusUseCase(new InMemoryOrderRepository(), new SimpleEventBus());
    const output = new RecordingOutput();

    expect(() => advanceOrder(useCase, output, 'no-such-order')).toThrow(ExitError);
    expect(output.errors[0]).toContain('OrderNotFound');
  });
});

describe('deliverOrder', () => {
  it('prints a confirmation on success', () => {
    const orderRepo = new InMemoryOrderRepository();
    const medicinalProductRepo = new InMemoryMedicinalProductRepository();
    const eventBus = new SimpleEventBus();
    medicinalProductRepo.save(new MedicinalProduct('prod-1' as MedicinalProductId, 'Paracetamol 500mg', 'med-1' as MedicationId, new Decimal(10), new Decimal(5)));
    const created = new CreateOrderUseCase(orderRepo, eventBus).execute({ actorId: 'cli', wardUnitId: 'ward-1' as WardUnitId, lines: [{ medicationId: 'med-1' as MedicationId, quantity: 5 }] });
    if (!created.successful) throw new Error('setup failed');
    new AdvanceOrderStatusUseCase(orderRepo, eventBus).execute({ actorId: 'cli', orderId: created.value.id });
    new AdvanceOrderStatusUseCase(orderRepo, eventBus).execute({ actorId: 'cli', orderId: created.value.id });
    const useCase = new DeliverOrderUseCase(orderRepo, medicinalProductRepo, eventBus);
    const output = new RecordingOutput();

    deliverOrder(useCase, output, created.value.id);
