...
medications
  ...
  .action((opts) => listMedications(medicationRepo, opts.query));

medications
  .command('show <id>')
  .description('Show a medication and its registered medicinal products')
  .action((id) => showMedication(medicationRepo, medicinalProductRepo, id));
...
orders
  ...
  .action((opts) => createOrder(createOrderUseCase, opts.wardUnitId, opts.medicationId, opts.quantity));

orders
  .command('advance <orderId>')
  .description('Advance an order to the next status')
  .action((orderId) => advanceOrder(advanceOrderStatusUseCase, orderId));

orders
  .command('deliver <orderId>')
  .description('Mark an order as delivered and update stock')
  .action((orderId) => deliverOrder(deliverOrderUseCase, orderId));
