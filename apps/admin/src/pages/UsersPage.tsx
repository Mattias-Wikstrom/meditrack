    ...
    <div>
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--surface)] rounded-xl shadow-xl border border-[var(--border)] p-6 w-full max-w-sm mx-4">
            <h2 className="text-base font-semibold text-[var(--ink)] mb-4">New User</h2>
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {sorted.map(actor => (
              <tr key={actor.id} onClick={() => navigate(`/users/${actor.id}`)} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-2)] cursor-pointer">
                <td className="py-3 px-4 font-medium text-[var(--ink)]">{actor.id}</td>
                <td className="py-3 px-4"><RoleBadge role={actor.role} /></td>
                <td className="py-3 px-4 text-[var(--text)]">{actor.wardUnit?.name ?? actor.wardUnitId ?? <span className="text-[var(--faint)]">—</span>}</td>
              </tr>
            ))}
            {sorted.length === 0 && (
              <tr>
                <td colSpan={3} className="py-12 text-center text-[var(--faint)]">No users found.</td>
              </tr>
            )}
          </tbody>
        </table>
